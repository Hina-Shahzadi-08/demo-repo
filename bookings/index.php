			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						...
						$qry = $conn->query("SELECT b.*,h.name as `hall`,concat(c.lastname,', ',c.firstname,' ', c.middlename) as fullname from `booking_list` b  inner join `hall_list` h on b.hall_id = h.id inner join client_list c on b.client_id = c.id where b.users_fk={$_settings->userdata('id')} order by b.status asc, unix_timestamp(b.`date_created`) desc ");
					...
					?>
						<tr>
							...
							<td class=""><?php echo date("Y-m-d",strtotime($row['event_date'])) ?></td>
