...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT b.*,h.name as `hall`,concat(c.firstname,', ',c.middlename,' ', c.lastname) as fullname, c.contact, c.address from `booking_list` b inner join `hall_list` h on b.hall_id = h.id inner join client_list c on b.client_id = c.id where b.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <div id="outprint" class="list-group">
        <div class="row">
            ...
            <div class="col-8 text-dark border"><?= isset($event_date) ? date("M d, Y", strtotime($event_date)) : "N/A" ?></div>
